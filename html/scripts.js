$(document).ready(function(){
  ...
  function playGame(DropAmount,letterSet,speed,intervalSet) {
    ...
    var score = 0
    var rate = speed;
    var RATE_INTERVAL = .01; //playbackRate will increase by .05 for each letter... so after 20 letters, the rate of falling will be 2x what it was at the start
    var misses = 0;
    ...
    function gameOver() {
      $.post('http://rpv-blackout/failure', JSON.stringify({}));
      gameOn = false;
      clearInterval(cleanupInterval);
      getAllAnimations().forEach(function(anim) {
        anim.pause();
    ...
    function setupNextLetter() {
      ...
      {
        soundSet = true
        $.post('http://rpv-blackout/complete', JSON.stringify({}));
      }
    }
    setupNextLetter();
  }
  ...
  document.onkeyup = function (data) {
      if (data.which == 27 ) {
        $.post('http://rpv-blackout/close', JSON.stringify({}));
      }
    };
